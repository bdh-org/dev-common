# Stack architecture patterns (P1–P14)
...
`stack-common/CLAUDE.md`.

This catalog used to live in `CLAUDE.md`, which loads into every Claude Code
session in every consumer repo. It was moved here verbatim in
bdh-org/dev-common#273 to keep that file under Claude Code's instruction-size
limit. `CLAUDE.md` keeps a one-line index, so `P10`-style references still
resolve. **Do not `@`-import this file from `CLAUDE.md`**: that would load it
at session start and count toward the limit again.
